...
✓ Nautical Mooring Rope & Deflector Dynamic Routing Engine [STATUS: SHIPPED & VERIFIED]:
  ...
  - Cross-Row Flanking Mooring Harness (`SeveredMooringPairCard` in `NauticalMooringRope.kt`): When severed into Separate Controls, the paired Upper and Lower rows are cradled in a continuous visual rope harness. The cut cords anchor above the Upper card, curve over the shoulders, dangle down both the left and right flanks across the gap into the Lower card, and swoop inward at the base with frayed hemp fibers. Tapping the overarching harness triggers the tactical confirmation to tie them back together.
