...
fun buildFlatItemsList(
                            ...
                            if (isCustomizable && isCustomExpanded) {
                                if (gravityBucket != null) {
                                    ...
                                    val subText = if (!isGravityEnabled) {
                                        ...
                                        "$assignedCount app(s) assigned • Tap to customize bucket apps"
                                    }
                                    list.add(
                                        PickerRowItem.SystemCustomizationOption(
                                            parentToken = token,
                                            optionKey = "gravity_bucket_assign:${gravityBucket.name}",
                                            title = if (!isGravityEnabled) "Assigned Apps: Engine Disabled" else "Assigned Apps: $assignedCount app${if (assignedCount == 1) "" else "s"}",
                                            subtitle = subText,
                                            isSelected = isGravityEnabled && assignedCount > 0
                                        )
                                    )
                                } else if (token == "system:media_skip_forward" || token == "system:media_skip_backward") {
    ...
    nonSystem.forEach { token ->
        val pkg = when {
            ...
            token.startsWith("shortcut:") -> {
                when {
                    token.contains(";pkg=") -> token.substringAfter(";pkg=").substringBefore(";")
                    token.contains(":pkg=") -> token.substringAfter(":pkg=").substringBefore(";")
                    token.contains("package=") -> token.substringAfter("package=").substringBefore(";")
                    token.contains(";act=") -> token.substringAfter(";act=").substringBefore(";").substringBeforeLast(".")
                    else -> "other"
                }
            }
            else -> "other"
    ...
    val sortedApps = appMap.keys.map { pkg ->
        val appLabel = if (pkg == "other") "Custom & Pinned Shortcuts" else (labelCache["app:$pkg"] ?: pkg)
    ...
    sortedApps.forEach { (triple, isPinned) ->
        ...
        val homeShortcuts = tokens.filter {
            it.contains("type=home_shortcut") ||
            it.contains("type=pinned") ||
            it.contains("type=intent") ||
            it.startsWith("shortcut:type=intent") ||
            it.startsWith("shortcut:type=pinned")
        }
