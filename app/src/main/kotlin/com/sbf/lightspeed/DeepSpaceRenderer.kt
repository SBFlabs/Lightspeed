...
/**
 * Encapsulates rendering state and callbacks for the Cockpit Hangar view.
 */
data class HangarRenderState(
    val canvas: Canvas,
    val context: Context,
    val screenW: Float,
    val screenH: Float,
    ...
    val m3Primary: Int,
    val m3Secondary: Int,
    val isOpenedFromLeftFlank: Boolean,
    val activeGearSetIndex: Int,
    val activeHangarRing: Int,
    val isHangarEjectArmed: Boolean,
    val gearRingRotations: FloatArray,
    val hangarBayScrollOffset: Float,
    val setsList: List<String>,
    val resolveCleanAppLabel: (String) -> String,
    val getAppsForActiveGear: (setIndex: Int, ringIndex: Int) -> List<String>,
    val getFlankLaunchBehavior: (Boolean) -> String,
    val onIconResolved: (() -> Unit)? = null
)
...
internal class DeepSpaceRenderer(
    ...
    val reactorRenderer = LightspeedReactorRenderer()

    // -------------------------------------------------------------------------
    // Warp launch state — delegated to LightspeedReactorRenderer
    // -------------------------------------------------------------------------
    var isWarpLaunching: Boolean
        get() = reactorRenderer.isWarpLaunching
        set(value) { if (!value) reactorRenderer.cancelWarp() }

    var warpStartTime: Long
        get() = reactorRenderer.warpStartTime
        set(_) {}

    var warpFocalPointX: Float
        get() = reactorRenderer.warpFocalPointX
        set(_) {}

    var warpFocalPointY: Float
        get() = reactorRenderer.warpFocalPointY
        set(_) {}
    ...
    /**
     * Renders the full COCKPIT_HANGAR screen onto canvas using [HangarRenderState].
     */
    fun drawDeepSpace(state: HangarRenderState) {
        val canvas = state.canvas
        val context = state.context
        val screenW = state.screenW
        val screenH = state.screenH
        val d = state.density
        val m3Primary = state.m3Primary
        val m3Secondary = state.m3Secondary
        val isOpenedFromLeftFlank = state.isOpenedFromLeftFlank
        val activeGearSetIndex = state.activeGearSetIndex
        val activeHangarRing = state.activeHangarRing
        val isHangarEjectArmed = state.isHangarEjectArmed
        val gearRingRotations = state.gearRingRotations
        val hangarBayScrollOffset = state.hangarBayScrollOffset
        val setsList = state.setsList
        val resolveCleanAppLabel = state.resolveCleanAppLabel
        val getAppsForActiveGear = state.getAppsForActiveGear
        val getFlankLaunchBehavior = state.getFlankLaunchBehavior
        val onIconResolved = state.onIconResolved
        ...
        for (g in setsList.indices) {
            ...
            val isBayLocked = (g >= 2 && !isInfinityUnlocked)
            val shortLabel = if (isBayLocked) "${g + 1}: 🔒" else "${g + 1}: ${setName.take(6)}"
            ...
            textPaint.color = if (isBayLocked) 0xFFFFD700.toInt() else if (isCurrentBay) Color.WHITE else Color.argb(170, 200, 220, 255)
        ...
        val isPlusLocked = (setsList.size >= 2 && !isInfinityUnlocked)
        highlightPaint.style = Paint.Style.FILL
        ...
        highlightPaint.color = if (isPlusLocked) 0xFFFFD700.toInt() else Color.argb(60, 200, 220, 255)
        canvas.drawRoundRect(plusRect, 10f * d, 10f * d, highlightPaint)
        textPaint.textSize = if (isPlusLocked) 12f * d else 15f * d
        textPaint.color = if (isPlusLocked) 0xFFFFD700.toInt() else Color.WHITE
        canvas.drawText(if (isPlusLocked) "🔒" else "+", plusBtnX, r2Y + (if (isPlusLocked) 4f * d else 5.5f * d), textPaint)
        ...
        drawPill(ret3Rect, if (isInfinityUnlocked) "+ CROSS" else "+ CROSS 🔒", reticleStyle == "cross")
        drawPill(ret4Rect, if (isInfinityUnlocked) "◇ DIAMOND" else "◇ DIAMOND 🔒", reticleStyle == "diamond")

        // ROW 8: RETICLE TARGETING HORIZON (CLASSIC 180° VS DYNAMIC FLANK)
        val r8Y = r7Y + 33f * d
        val oriW = (deckW - gap) / 2f
        val ori1Rect = RectF(leftX, r8Y - 14f * d, leftX + oriW, r8Y + 14f * d)
        val ori2Rect = RectF(rightX - oriW, r8Y - 14f * d, rightX, r8Y + 14f * d)

        val reticleOri = prefs.getString(com.sbf.lightspeed.system.LightspeedPreferences.KEY_COCKPIT_RETICLE_ORIENTATION, com.sbf.lightspeed.system.LightspeedPreferences.RETICLE_ORIENTATION_CLASSIC_180) ?: com.sbf.lightspeed.system.LightspeedPreferences.RETICLE_ORIENTATION_CLASSIC_180
        drawPill(ori1Rect, "🎯 180° HORIZON", reticleOri == com.sbf.lightspeed.system.LightspeedPreferences.RETICLE_ORIENTATION_CLASSIC_180)
        drawPill(ori2Rect, "🔄 DYNAMIC FLANK", reticleOri == com.sbf.lightspeed.system.LightspeedPreferences.RETICLE_ORIENTATION_MIRRORED_FLANK)

        // 8. Live Dual Gimbal Assembly Preview & Active Ring Command Core
        val cyGimbal = (r8Y + 155f * d).coerceAtLeast(screenH * 0.63f)
        val radOuter = 135f * d
        val radInner = 84f * d
        val radHub = 32f * d
        ...
        val targetFocusAngle = if (reticleOri == com.sbf.lightspeed.system.LightspeedPreferences.RETICLE_ORIENTATION_MIRRORED_FLANK && isOpenedFromLeftFlank) 0f else 180f
        ...
        for (r in 0..1) {
            ...
            for (i in ringApps.indices) {
                ...
                val angleDiff = kotlin.math.abs(normalizedDeg - targetFocusAngle).let { if (it > 180f) 360f - it else it }
                ...
                try {
                    val dIcon = com.sbf.lightspeed.system.LightspeedIconManager.getCachedIconDrawable(itemToken)
                    if (dIcon != null) {
                        ...
                        // Tactical Monogram Placeholder: draws instantly in 0ms without blocking onDraw
                        val initial = appLabel.firstOrNull()?.uppercase() ?: "?"
                        textPaint.color = Color.WHITE
                        textPaint.textSize = currentSize * 0.44f
                        textPaint.typeface = android.graphics.Typeface.DEFAULT_BOLD
                        textPaint.textAlign = Paint.Align.CENTER
                        canvas.drawText(initial, iconCX, iconCY + (currentSize * 0.16f), textPaint)
        ...
        // Targeting Reticle Collimator at targetFocusAngle (180° Inward Horizon or 0° Dynamic Flank)
        val ringRad = if (activeHangarRing == 0) radOuter else radInner
        val reticleX = if (targetFocusAngle == 0f) cx + ringRad else cx - ringRad
    ...
    fun drawDeepSpace(
        ...
        onIconResolved: (() -> Unit)? = null
    ) {
        drawDeepSpace(
            HangarRenderState(
                canvas, context, screenW, screenH, d, m3Primary, m3Secondary,
                isOpenedFromLeftFlank, activeGearSetIndex, activeHangarRing,
                isHangarEjectArmed, gearRingRotations, hangarBayScrollOffset,
                setsList, resolveCleanAppLabel, getAppsForActiveGear,
                getFlankLaunchBehavior, onIconResolved
            )
        )
    }
    ...
    fun drawHyperdriveWarpSurge(canvas: Canvas, m3Primary: Int, density: Float) {
        reactorRenderer.drawHyperdriveWarpSurge(canvas, m3Primary, density)
    }
    ...
    fun drawHolographicReactorCore(
        ...
        reactorRenderer.drawHolographicReactorCore(canvas, cx, cy, coreRadius, isActive, m3Primary, density)
    }

    fun drawCosmicStarfield(canvas: Canvas, w: Float, h: Float, density: Float, alphaFactor: Float) {
        cosmicRenderer.drawCosmicStarfield(canvas, w, h, density, alphaFactor)
    }
    ...
    fun drawGalacticNebula(
        ...
        cosmicRenderer.drawGalacticNebula(canvas, cx, cy, radius, m3Primary, alphaFactor)
    }
}
