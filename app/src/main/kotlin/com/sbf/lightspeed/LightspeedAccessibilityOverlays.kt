...
internal fun LightspeedAccessibilityService.setupStatusBarOverlay(prefs: SharedPreferences) {
    ...
    val sensorThicknessDp = (if (isLandscape) prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_THICKNESS_LANDSCAPE, 35) else prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_THICKNESS, 35)).coerceIn(10, 52)
...
internal fun LightspeedAccessibilityService.setupSensorTouchOverlay(prefs: SharedPreferences) {
    ...
    val density = resources.displayMetrics.density
    val isLandscape = resources.configuration.orientation == android.content.res.Configuration.ORIENTATION_LANDSCAPE
    val spanPref = if (isLandscape) prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_SPAN_LANDSCAPE, 150) else prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_SPAN, 200)
    val spanPx = if (spanPref >= 1000) screenW else (spanPref * density).coerceIn(50f * density, screenW)
    val sensorOffsetX = (if (isLandscape) prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_OFFSET_X_LANDSCAPE, 300) else prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_OFFSET_X, 105)) * density
    val thicknessDp = (if (isLandscape) prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_THICKNESS_LANDSCAPE, 35) else prefs.getInt(LightspeedPreferences.KEY_STATUSBAR_THICKNESS, 35)).coerceIn(10, 52)
...
internal fun LightspeedAccessibilityService.updateStatusBarOverlayFromPrefs(prefs: SharedPreferences) {
    ...
    if (statusBarOverlayView == null) {
        ...
        statusBarOverlayView?.updateRenderCache()
...
internal fun LightspeedAccessibilityService.setupOrientationAnchor() {
    ...
    try {
        ...
        Log.i("LightspeedAccessibility", "Initialized hardware orientation anchor window (TYPE_ACCESSIBILITY_OVERLAY)")
    } catch (e: Exception) {
...
internal fun LightspeedAccessibilityService.updateForcedOrientationInternal(orientation: Int) {
    handler.post {
        ...
        if (anchor == null || params == null) {
            if (orientation == ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED) return@post
