...
class LightspeedNotchOverlay(context: Context) : View(context) {
    ...
    data class NotchRenderCache(
        var isCapsuleEnabled: Boolean = false,
        var dlRouting: String = "notch_pill",
        var mediaRouting: String = "none",
        var isTestBeacon: Boolean = false,
        var capsuleWidgetId: Int = -1,
        var capsuleOrientMode: String = "both",
        var capsuleLayout: String = "unified_right",
        var scrollMode: String = "loop_2x",
        var truncateAnchor: String = "tail",
        var expansionW: Int = 0,
        var marqueeSpeed: Int = 30,
        var snugnessDp: Int = 2,
        var customCutoutWidth: Int = -1,
        var cutoutOffsetX: Int = 0,
        var cutoutOffsetY: Int = 0
    )

    internal var renderCache = NotchRenderCache()

    internal fun updateRenderCache() {
        renderCache = NotchRenderCache(
            isCapsuleEnabled = prefs.getBoolean(LightspeedPreferences.KEY_ORBITAL_CAPSULE_ENABLED, false),
            dlRouting = prefs.getString(LightspeedPreferences.KEY_RAIL_DOWNLOADS_ROUTING, "notch_pill") ?: "notch_pill",
            mediaRouting = prefs.getString(LightspeedPreferences.KEY_RAIL_MEDIA_ROUTING, "none") ?: "none",
            isTestBeacon = prefs.getBoolean(LightspeedPreferences.KEY_NOTCH_TEST_BEACON, false),
            capsuleWidgetId = prefs.getInt(LightspeedPreferences.KEY_CAPSULE_WIDGET_ID, -1),
            capsuleOrientMode = prefs.getString(LightspeedPreferences.KEY_NOTCH_CAPSULE_ORIENTATION_MODE, "both") ?: "both",
            capsuleLayout = prefs.getString(LightspeedPreferences.KEY_NOTCH_CAPSULE_LAYOUT, "unified_right") ?: "unified_right",
            scrollMode = prefs.getString(LightspeedPreferences.KEY_NOTCH_TEXT_SCROLL_MODE, "loop_2x") ?: "loop_2x",
            truncateAnchor = prefs.getString(LightspeedPreferences.KEY_NOTCH_TEXT_TRUNCATE_ANCHOR, "tail") ?: "tail",
            expansionW = prefs.getInt(LightspeedPreferences.KEY_NOTCH_EXPANSION_WIDTH, 0),
            marqueeSpeed = prefs.getInt(LightspeedPreferences.KEY_NOTCH_MARQUEE_SPEED, 30),
            snugnessDp = prefs.getInt(LightspeedPreferences.KEY_NOTCH_PADDING_SNUGNESS, 2),
            customCutoutWidth = prefs.getInt(LightspeedPreferences.KEY_NOTCH_CUTOUT_WIDTH, -1),
            cutoutOffsetX = LightspeedPreferences.getEffectiveCutoutOffsetX(prefs),
            cutoutOffsetY = LightspeedPreferences.getEffectiveCutoutOffsetY(prefs)
        )
    }
    ...
    internal val prefListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
        if (key != null && (key.startsWith("pref_notch_") || key.startsWith("pref_telemetry_") || key == "pref_statusbar_enabled" || key.startsWith("pref_rail_") || key == LightspeedPreferences.KEY_ORBITAL_CAPSULE_ENABLED || key == LightspeedPreferences.KEY_CAPSULE_WIDGET_ID)) {
            updateRenderCache()
            currentTextKey = ""
    ...
    fun updateCapsuleLayout() {
        ...
        val capsuleOrientMode = renderCache.capsuleOrientMode
        ...
        val hardwareCutoutWidthDp = if (renderCache.customCutoutWidth > 0) renderCache.customCutoutWidth else detectedCutoutWidthDp
        ...
        val offsetX = renderCache.cutoutOffsetX.toFloat() * d
        val offsetY = renderCache.cutoutOffsetY.toFloat() * d
        val expansionW = renderCache.expansionW * d
        val snugnessDp = renderCache.snugnessDp
        val snugnessPx = snugnessDp * d
        val capsuleLayout = renderCache.capsuleLayout
    ...
    internal fun getCircularAppIcon(packageName: String, sizePx: Int): Bitmap? {
        ...
        val cacheKey = "notch_circle:$packageName:$sizePx"
        LightspeedIconManager.getCachedBitmap(cacheKey)?.let { return it }
        ...
        return try {
            ...
            LightspeedIconManager.putCachedBitmap(cacheKey, bitmap)
            bitmap
        } catch (_: Exception) {
            null
        }
