...
object LightspeedBackupEngine {
    ...
    private val TRANSIENT_KEYS = setOf(
        LightspeedPreferences.KEY_LAST_ACTIVE_SET_INDEX,
        LightspeedPreferences.KEY_LAST_ACTIVE_SET_INDEX_LEFT,
        LightspeedPreferences.KEY_LAST_ACTIVE_SET_INDEX_RIGHT,
        LightspeedPreferences.KEY_SIDEBAR_LEFT_PREVIEW,
        LightspeedPreferences.KEY_SIDEBAR_RIGHT_PREVIEW,
        LightspeedPreferences.KEY_STATUSBAR_PREVIEW,
        LightspeedPreferences.KEY_NOTCH_TEST_BEACON,
        LightspeedPreferences.KEY_HORIZON_RAIL_PREVIEW,
        "pref_is_infinity_unlocked",
    ...
    fun importFromJson(context: Context, jsonString: String): Result<Int> {
        return try {
            ...
            if (backupVersion < BACKUP_VERSION) {
                Log.w(TAG, "Importing legacy backup version $backupVersion (current: $BACKUP_VERSION). Running migration.")
                migrateFromV1(editor)
            }
            ...
            while (keys.hasNext()) {
                ...
                if (!declaredType.isNullOrEmpty()) {
                    when (declaredType) {
                        "bool" -> editor.putBoolean(key, if (value is Boolean) value else value.toString().toBoolean())
                        "int" -> editor.putInt(key, if (value is Number) value.toInt() else value.toString().toIntOrNull() ?: 0)
                        "long" -> editor.putLong(key, if (value is Number) value.toLong() else value.toString().toLongOrNull() ?: 0L)
                        "float" -> editor.putFloat(key, if (value is Number) value.toFloat() else value.toString().toFloatOrNull() ?: 0f)
                ...
                // Heuristic fallback for legacy backups without declared type
                ...
                when (value) {
                    ...
                    is Number -> {
                        if (existingVal is Float || value is Double || key == LightspeedPreferences.KEY_BACK_TAP_THRESHOLD || key.contains("threshold") || key.contains("scale") || key.contains("friction") || key.contains("sensitivity") || key.contains("velocity")) {
                            editor.putFloat(key, value.toFloat())
                        } else if (existingVal is Long || key.endsWith("_time") || key.endsWith("_timestamp")) {
                    ...
                    is String -> {
                        if (existingVal is Boolean) {
                            editor.putBoolean(key, value.toBoolean())
                        } else if (existingVal is Float || key == LightspeedPreferences.KEY_BACK_TAP_THRESHOLD) {
                            val floatVal = value.toFloatOrNull()
                            if (floatVal != null) editor.putFloat(key, floatVal) else editor.putString(key, value)
                        } else if (existingVal is Int || key.startsWith("pref_hardware_") || key.startsWith("pref_sidebar_") || key.startsWith("pref_statusbar_") || key.startsWith("pref_notch_") || key.startsWith("pref_horizon_rail_") || key.startsWith("last_active_set_index") || key == LightspeedPreferences.KEY_REFUELING_WIDGET_ID || key == LightspeedPreferences.KEY_CAPSULE_WIDGET_ID || key == LightspeedPreferences.KEY_REFUELING_STACK_LAST_PAGE || key == LightspeedPreferences.KEY_MEDIA_SKIP_SECONDS) {
                            val intVal = value.toIntOrNull()
                            if (intVal != null) editor.putInt(key, intVal) else editor.putString(key, value)
                        } else if (existingVal is Long || key.endsWith("_time") || key.endsWith("_timestamp")) {
                            val longVal = value.toLongOrNull()
                            if (longVal != null) editor.putLong(key, longVal) else editor.putString(key, value)
                        } else {
                            if (value == "true" || value == "false") {
                                if (key.startsWith("pref_") && (key.endsWith("_enabled") || key.endsWith("_demoted") || key.endsWith("_armed") || key.endsWith("_guard") || key.endsWith("_unlocked") || key.endsWith("_shadow") || key.endsWith("_glow") || key.endsWith("_caps") || key.endsWith("_preview") || key.endsWith("_dismissed") || key.endsWith("_lockscreen"))) {
                                    editor.putBoolean(key, value.toBoolean())
                                } else {
            ...
            if (iconsObject != null) {
                try {
                    ...
                    while (iconKeys.hasNext()) {
                        ...
                        if (key.matches(Regex("^-?[0-9a-zA-Z_]+$"))) {
                            val b64 = iconsObject.optString(key, "")
                            if (b64.isNotBlank()) {
            ...
            try {
                CockpitGearRepository.clearCache()
