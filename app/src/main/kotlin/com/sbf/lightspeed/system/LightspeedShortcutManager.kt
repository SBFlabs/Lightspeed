...
object LightspeedShortcutManager {
    ...
    fun saveShortcutBitmap(context: Context, token: String, bitmap: Bitmap) {
        ...
        LightspeedIconManager.putCachedBitmap(token, bitmap)
    ...
    fun loadCachedBitmap(context: Context, token: String): Bitmap? {
        ...
        LightspeedIconManager.getCachedBitmap(token)?.let {
            if (!isCorruptBitmap(it)) return it
            LightspeedIconManager.removeCachedBitmap(token)
        }
        try {
            ...
            if (file.exists()) {
                ...
                if (bmp != null) {
                    ...
                    LightspeedIconManager.putCachedBitmap(token, bmp)
                    return bmp
                }
            }
        } catch (_: Exception) {}
    ...
    fun resolveLabel(context: Context, token: String): String {
        ...
        labelCache[token]?.let { return it }

        val resolved = resolveLabelInternal(context, token)
        if (resolved.isNotBlank()) {
            labelCache[token] = resolved
        }
        return resolved
    }

    private fun resolveLabelInternal(context: Context, token: String): String {
        try {
    ...
    fun resolveIconBitmap(context: Context, token: String, useCache: Boolean = true): Bitmap? {
        if (token.isBlank() || token == "none") return null
        if (useCache) LightspeedIconManager.getCachedBitmap(token)?.let { return it }
        ...
        if (d is BitmapDrawable && d.bitmap != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && bmp.config == Bitmap.Config.HARDWARE) {
                ...
                if (useCache) LightspeedIconManager.putCachedBitmap(token, softwareBmp) else temporaryPickerCache[token] = softwareBmp
                return softwareBmp
            }
            if (useCache) LightspeedIconManager.putCachedBitmap(token, bmp) else temporaryPickerCache[token] = bmp
            return bmp
        }
    ...
    fun launch(context: Context, token: String): Boolean {
        ...
        when (parsed.type) {
            "pinned", "home_shortcut" -> {
                ...
                if (!launched) {
                    launched = ShizukuShortcutLauncher.launchElevatedHomeShortcut(context, parsed)
                }
